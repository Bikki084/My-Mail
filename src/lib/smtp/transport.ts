...
import { isSesSmtpHost, isBrevoSmtpHost, isMailjetSmtpHost, isSendGridSmtpHost, isZohoSmtpHost } from "@/lib/smtp/from-address";
...
export function buildSmtpUserTransport(v: {
  ...
  const dkim =
    isSesSmtpHost(v.host) || isBrevoSmtpHost(v.host) || isMailjetSmtpHost(v.host) || isSendGridSmtpHost(v.host) || isZohoSmtpHost(v.host) ? null : getDkimConfigFromEnv();
