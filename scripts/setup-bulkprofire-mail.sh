#!/usr/bin/env bash
# Configure Postfix on this VPS for bulkprofire.com (local relay on 127.0.0.1:25).
# Run on the server: sudo bash scripts/setup-bulkprofire-mail.sh
set -euo pipefail

DOMAIN="${MAIL_DOMAIN:-bulkprofire.com}"
...
AWS / DO may block OUTBOUND port 25 to Gmail/Yahoo. If mail does not deliver,
request removal with your cloud provider or use Brevo SMTP on port 587 instead.
============================================================
EOF
